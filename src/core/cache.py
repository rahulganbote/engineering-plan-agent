...
def get_semantic_backend(namespace: str) -> "Optional[SemanticBackend]":
...
def reset_default_backend(backend: Optional[CacheBackend] = None) -> None:
            ...
            if policy.mode == "semantic" and getattr(be, "semantic_get", None) is not None:
                ...
                if query_text:
                    ...
                    if hit is not None:
                        ...
                        _emit("cache_hit",
                              namespace=policy.namespace, call=call_name,
                              mode="semantic", key=key[:8])
                        return hit
                    log.debug(f"[cache:{policy.namespace}] SEMANTIC MISS {call_name}")
                    _emit("cache_miss",
                          namespace=policy.namespace, call=call_name, key=key[:8])
...
class RedisCache:
    ...
    def __init__(self, url: str, key_prefix: str = "em-copilot", socket_timeout: float = 1.0):
        try:
            ...
            raise RuntimeError(
                ...
                "Add `redis>=5.0` to requirements.txt and reinstall."
            ) from e
    ...
    def clear(self, namespace: str = "") -> None:
        try:
            ...
            log.warning(f"[cache:redis] clear failed: {e}")
...
class TieredCache:
    ...
    def clear(self, namespace: str = "") -> None:
        try: self._l1.clear(namespace)
        except Exception: pass
        try: self._l2.clear(namespace)
        except Exception: pass
...
class SemanticBackend:
    ...
    def semantic_get(self, query_text: str, namespace: str, threshold: float) -> Optional[Any]:
        """
            ...
            if results.matches and results.matches[0].score >= th:
                ...
                if cached_value is not None:
                    ...
                    try:
                        ...
                        value = cached_value
                    log.debug(
                        f"[cache:semantic] HIT ns={ns} score={match.score:.3f}"
                    )
                    self._hits += 1
                    return value
            self._misses += 1
