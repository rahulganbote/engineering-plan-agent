...
Design principle:
    ...
    important field in the system — citations directly map to rubric pts
    for "Grounding & Citation quality" (8 pts).
...
Rubric coverage:
    - Structured Output Contracts (10 pts): schema design 5pts
    - RAG Grounding (8 pts):               citations field on every output
    - Critic Agent (15 pts):               CriticOutput + DimensionScore
"""
...
class QualityBadge(str, Enum):
    ...
    Displayed in the Streamlit UI after pipeline completion.

    Thresholds (from capstone rubric):
...
class HITLDecision(str, Enum):
    """
    Human-in-the-loop decision recorded via Streamlit approve/reject button.
...
class AgentOutputBase(BaseModel):
    ...
    citations: list[str] = Field(
        ...
        min_length=1,
        description=(
            "Pinecone chunk IDs retrieved and used to ground this output. "
            "Required on every agent output. Maps to rubric: grounding = 8 pts."
        ),
    )
    ...
    def citations_must_not_be_empty(cls, v: list[str]) -> list[str]:
        ...
        if not v:
            raise ValueError(
                "citations must contain at least one Pinecone chunk ID. "
                "Agent output is not grounded."
            )
        return v
...
class ArchitectureOutput(AgentOutputBase):
    ...
    # ── Diagram (rubric: Architecture & PoC design, demo polish) ─────────────
    ...
    # diagram_svg is the Kroki-rendered SVG cached for direct Streamlit display.
...
class DimensionScore(BaseModel):
    """
    Score for a single Critic rubric dimension.
    Used to build the before/after improvement table required by rubric.
    """
...
class HallucinationFlag(BaseModel):
    ...
    Rubric: guardrails / hallucination control = 4 pts.
    """
    agent:               str
    claim:               str
...
class CriticOutput(BaseModel):
    ...
    The 4 rubric dimensions and their thresholds:
    ...
    to produce the before/after improvement table required by rubric (5 pts).
    """
...
class PipelineState(BaseModel):
    ...
    Eval tracking:
        ...
        in the Streamlit eval dashboard (rubric: revision loop = 5 pts).
    """
    ...
    plan_output:     Optional[EngineeringPlanOutput] = None
    schedule_output: Optional[ScheduleOutput]        = None
    arch_output:     Optional[ArchitectureOutput]    = None
    poc_output:      Optional[PoCOutput]             = None
    stack_output:    Optional[TechStackOutput]       = None
    ...
    critic_scores_history: list[dict] = Field(
        ...
        description=(
            ...
            "Used to generate before/after score comparison table in Streamlit. "
