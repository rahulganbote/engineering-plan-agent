...
def _run_specialists_in_parallel(ps: PipelineState, target_agents: list[str]) -> dict:
    ...
    # 1. Specialists read exclusively from the shared, thread-safe `PipelineState`
    #    instance. All fields are accessed as read-only copies during execution.
    # 2. No specialist is permitted to modify the shared `PipelineState` fields
    #    in-place within their respective threads.
    # 3. All outputs generated by specialists are returned as decoupled result objects
    #    to this coordinating main thread, which serializes and writes them back
    #    to the PipelineState object after all futures complete.
    # This separation guarantees no write-write or read-write data races.
    # ──────────────────────────────────────────────────────────────────────────
...
def node_pass1_drafting(state: dict) -> dict:
    ...
    _set_status(ps, PipelineStatus.DRAFTING)
    ...
    try:
        ...
        # Rationale for Mid-Pass-1 Output Population:
        # Standard output fields are populated immediately with Pass 1 drafts to:
        # 1. Provide a base state for intermediate cross-agent consistency validations
        #    executed during the Arbitration phase.
        # 2. Serve as a final fallback baseline if Pass 2 (alignment) is bypassed
        #    entirely due to zero consistency conflicts.
        # In Pass 2, targeted rerun results will selectively overwrite these fields.
...
def node_arbitrate(state: dict) -> dict:
    ...
    _set_status(ps, PipelineStatus.ARBITRATING)
    _safe_emit("agent_start", agent="orchestrator")

    try:
        from src.agents.critic.consistency_rules import check_cross_agent_consistency

        # Cost-Saver Optimization: First evaluate deterministic consistency rules.
        # If there are no conflicts, bypass the expensive LLM-based arbitrate_drafts.
        conflicts = check_cross_agent_consistency(ps)
        if not conflicts:
            log.info(f"[{ps.run_id}] Consistency checks passed with 0 conflicts. Skipping LLM arbitration.")
            from src.core.models import AlignmentMemo

            memo = AlignmentMemo(
                overall_strategy="Consistency validation passed. No alignment issues identified.",
                directives=[],
            )
            state["_revision_targets"] = []
        else:
            log.info(
                f"[{ps.run_id}] Consistency checks flagged {len(conflicts)} conflicts. Initiating LLM arbitration."
            )
            memo = OrchestratorAgent().arbitrate_drafts(ps)
            state["_revision_targets"] = [d.agent_name for d in memo.directives]
...
def node_pass2_alignment(state: dict) -> dict:
    ...
    _set_status(ps, PipelineStatus.ALIGNING)

    ps.pass_number = 2

    # Rerun targeted revision agents
    targets = state.get("_revision_targets", [])
    if targets:
        try:
        ...
        log.info(f"[{ps.run_id}] No targeted agents for alignment. Skipping Pass 2 rerun.")
