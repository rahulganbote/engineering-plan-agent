...
_SAFE_CONCEPT_KEYWORDS = {
    # NFR concepts
    "availability", "latency", "throughput", "scalability", "reliability",
    "security", "compliance", "consistency", "durability", "concurrency",
    # Architecture patterns
    "microservices", "monolith", "event-driven", "serverless", "rest",
    "graphql", "grpc", "websocket", "streaming", "batch",
    # Data concerns
    "transactional", "analytical", "real-time", "offline", "sync", "async",
    # Industry verticals (generic, non-customer-specific)
    "payments", "healthcare", "saas", "fintech", "edtech", "iot",
}
...
def build_tavily_query(agent_role: str, brd_sections: list, max_keywords: int = 5) -> str:
    ...
    section_blobs = [
        (getattr(s, "content", "") or "")[:60] for s in (brd_sections or [])
    ]
...
def tavily_search(query: str, max_results: int = 3) -> ToolResult:
    ...
    if not allowed:
        log.warning(
            f"Tavily monthly budget exhausted | used={used} budget={budget} "
            f"month={_BUDGET_STATE['month']}"
        )
        emit(
        ...
        return ToolResult(
            content=(
                f"Web search unavailable — Tavily monthly budget reached "
                f"({used}/{budget}). Resets next month."
            ),
            used_fallback=True,
            sources=[],
            trust_level="low",
