...
Public surface:
    build_artifacts_pdf(state: PipelineState) -> bytes
        ...
        GET /download/{run_id} endpoint and from the Streamlit "Download PDF"
...
Design choices:
    ...
    - The Mermaid diagram is rendered as a TEXT code block, not an embedded
      ...
      The Streamlit UI already shows the rendered SVG separately.
"""
...
def _styles():
    ...
    base.add(ParagraphStyle(
        name="Mono", parent=base["BodyText"], fontName="Courier",
        fontSize=8, leading=10, textColor=colors.HexColor("#111827"),
        leftIndent=8, rightIndent=8, backColor=colors.HexColor("#F3F4F6"),
...
_DEFAULT_TABLE_STYLE = TableStyle([
    ...
    ("ROWBACKGROUNDS", (0, 1), (-1, -1),
                    [colors.whitesmoke, colors.HexColor("#FAFBFC")]),
])
...
def build_artifacts_pdf(state: PipelineState) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"EM Copilot — {state.run_id}", author="EM Copilot",
    )
    styles = _styles()
    story: list[Any] = []
...
def _add_header(story, styles, state: PipelineState) -> None:
    ...
    story.append(Paragraph(
        f"<font color='#6B7280'>Multi-agent BRD-to-engineering-plan pipeline</font>",
        styles["BodyText"],
    ))
...
def _add_critic(story, styles, state: PipelineState) -> None:
    ...
    for name, dim in dims:
        rows.append([
            name, f"{dim.score:.2f}", f"≥ {dim.threshold:.2f}",
    ...
    if critic.consistency_issues:
        ...
        for issue in critic.consistency_issues[:6]:
            ...
            story.append(Paragraph(
                f"• <i>{issue.severity.value}</i> — {issue.conflict_description} "
                f"({agents})",
                styles["Small"],
            ))
    ...
    for phase in plan.phases:
        ...
        if phase.milestones:
            ...
            for m in phase.milestones:
                ms_rows.append([
                    str(m.week), m.name, m.owner_role,
    ...
    if plan.risks:
        ...
        for r in plan.risks:
            rrows.append([
                ...
                r.likelihood.value, r.impact.value,
...
def _add_architecture(story, styles, state: PipelineState) -> None:
    ...
    story.append(Paragraph(
        f"<b>Pattern:</b> {arch.pattern}  ·  "
        f"<b>Deployment:</b> {arch.deployment_model}",
        styles["BodyText"],
    ))
    if arch.pattern_justification:
        story.append(Paragraph(f"<b>Justification:</b> {arch.pattern_justification}",
                                styles["BodyText"]))
...
def _add_poc(story, styles, state: PipelineState) -> None:
    ...
    story.append(Paragraph(f"<b>Hypothesis:</b> {poc.poc_hypothesis}",
                            styles["BodyText"]))
    story.append(Paragraph(
    ...
    if poc.risk_if_poc_fails:
        story.append(Paragraph(f"<b>Risk if PoC fails:</b> {poc.risk_if_poc_fails}",
                                styles["Small"]))
    story.append(Spacer(1, 6))
...
def _add_tech_stack(story, styles, state: PipelineState) -> None:
    ...
    if stack.recommendation_rationale:
        story.append(Paragraph(f"<b>Rationale:</b> {stack.recommendation_rationale}",
                                styles["BodyText"]))
    ...
    if stack.options:
        ...
        tbl = Table(rows, colWidths=[1.7*inch, 0.7*inch, 0.7*inch, 0.6*inch,
