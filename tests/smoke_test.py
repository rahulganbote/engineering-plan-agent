...
def _():
    ...
    assert settings.rag_similarity_threshold == 0.45, f"Got {settings.rag_similarity_threshold}"
...
def _():
    from src.core.models import EngineeringPlanOutput, Milestone, Phase, Risk, RiskLevel
...
def _():
    ...
    try:
        EngineeringPlanOutput(
            ...
            risks=[
                Risk(description="R", likelihood=RiskLevel.LOW, impact=RiskLevel.LOW, mitigation="M", citation="c0")
            ],
            team_composition={"E": 1},
...
def _():
    ...
    required = ["start_timer", "elapsed_ms", "retrieve_context", "has_no_rag_hits", "_call_llm_with_retry", "log_run"]
    for m in required:
...
def _():
    ...
    assert any(term in plan_flags[0].claim.lower() for term in ("quantum", "cryptography", "sidecar", "telemetry"))
...
def _():
    ...
    from src.agents.critic import CriticAgent
    from src.agents.tech_stack import TechStackAgent
    from src.core.models import RiskLevel, StackOption
...
def _():
    from src.agents.pipeline import _graph
...
def _():
    from src.security.validator import ValidationStatus
...
"""
    ...
    assert result.status == ValidationStatus.BLOCKED, f"Injection should be BLOCKED, got {result.status}"
...
def _():
    from src.core.resilience import BreakerState, CircuitBreaker
...
def _():
    from src.core.resilience import CallPolicy, resilient
...
def _():
    from src.core.resilience import CallPolicy, CircuitBreaker, CircuitOpenError, resilient
...
def _():
    from src.core.cache import CACHE_EMBEDDING, CACHE_LLM, CACHE_RAG, CachePolicy
...
def _():
    from src.core.cache import CachePolicy, InMemoryCache, cached
...
def _():
    ...
    class BrokenL2:
        ...
        def set(self, k, v, ttl_sec, namespace):
...
def _():
    """No REDIS_URL → InMemoryCache (fallback legacy behavior when Redis is unavailable)."""
...
def _():
    from src.agents.registry import SPECIALISTS, get_specialist, register_specialist
...
def _():
    from src.agents.base_agent import BaseAgent
    from src.core.cache import CACHE_LLM, CachePolicy
    from src.core.resilience import OPENAI_POLICY, CallPolicy
...
def _():
    from src.core.providers import OpenAIProvider, get_provider
...
def _():
    from src.core.providers import AnthropicProvider, get_provider
...
@test(
    "Providers: calculate_cost('anthropic', 'claude-sonnet-4-5', 1000, 1000) returns non-zero float", group="providers"
)
def _():
